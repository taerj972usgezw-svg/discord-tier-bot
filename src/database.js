...
module.exports = {
  ...
  updateUserInfo: (id, nickname, realname, style) => {
    const stmt = db.prepare(
      'UPDATE users SET nickname = COALESCE(?, nickname), realname = COALESCE(?, realname), style = COALESCE(?, style) WHERE id = ?'
    );
  ...
  applyLadderWin: (challengerId, defenderId) => {
    const runTransaction = db.transaction(() => {
      ...
      db.prepare(
        'UPDATE users SET rank = rank + 1 WHERE rank >= ? AND rank < ?'
      ).run(dRank, cRank);
  ...
  saveTierMessage: (tier, messageId, channelId) => {
    db.prepare(
      'INSERT INTO tier_messages (tier, message_id, channel_id) VALUES (?, ?, ?) ' +
